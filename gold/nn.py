...
AUX_FEATURE_COLUMNS = ("ret1", "ret1_lag1", "close_in_range", "atr14_rel", "ret8")
MODEL_FEATURE_COUNT = len(GOLD_FEATURE_COLUMNS) + 2 * len(AUX_FEATURE_COLUMNS)


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train the minimal GOLD Mamba pipeline.")
    ...
    parser.add_argument("--epochs", type=int, default=28, help="Maximum training epochs.")
    parser.add_argument("--batch-size", type=int, default=32, help="Training batch size.")
    parser.add_argument("--max-train-windows", type=int, default=3072, help="Training window cap.")
    parser.add_argument("--max-eval-windows", type=int, default=1024, help="Validation/test window cap.")
    parser.add_argument(
        "--imbalance-min-ticks",
        type=int,
        default=180,
...
def build_aligned_bars(
    ...
    t0 = time.time()
    df_all = pd.read_csv(csv_path, usecols=["symbol", "time_msc", "bid", "ask"])
    df_all["symbol"] = df_all["symbol"].astype(str).str.upper()

    df_gold = df_all[df_all["symbol"] == SYMBOL_ORDER[0]].sort_values("time_msc").reset_index(drop=True)
    if df_gold.empty:
        raise ValueError(f"No ticks found for {SYMBOL_ORDER[0]}")
    ...
    bar_ends = df_gold.groupby("bar_id")["time_msc"].last().to_numpy()
    gold_time_open = df_gold.groupby("bar_id")["time_msc"].first()

    bars: dict[str, pd.DataFrame] = {}
    for symbol in SYMBOL_ORDER:
        df_symbol = df_all[df_all["symbol"] == symbol].sort_values("time_msc").reset_index(drop=True)
        if df_symbol.empty:
            raise ValueError(f"No ticks found for {symbol}")

        if symbol == SYMBOL_ORDER[0]:
            df_binned = df_gold.copy()
        else:
            bar_ids = np.searchsorted(bar_ends, df_symbol["time_msc"].to_numpy(dtype=np.int64, copy=False), side="left")
            valid = bar_ids < len(bar_ends)
            df_binned = df_symbol.loc[valid].copy()
            df_binned["bar_id"] = bar_ids[valid]

        df_binned["tick_sign"] = compute_tick_signs(df_binned["bid"].to_numpy(dtype=np.float64, copy=False))
        df_binned["spread"] = df_binned["ask"] - df_binned["bid"]
        spread_last = df_binned.groupby("bar_id")["spread"].last()
        grouped = df_binned.groupby("bar_id").agg(
            {
        ...
        grouped.columns = ["open", "high", "low", "close", "time_open", "tick_imbalance", "ask_high", "ask_low"]
        grouped["spread"] = spread_last
        grouped = grouped.reindex(np.arange(len(bar_ends)))
        ...
        grouped["spread"] = grouped["spread"].ffill().bfill().fillna(0.0)
        grouped["ask_high"] = grouped["ask_high"].fillna(grouped["high"] + grouped["spread"])
        grouped["ask_low"] = grouped["ask_low"].fillna(grouped["low"] + grouped["spread"])
        grouped["tick_imbalance"] = grouped["tick_imbalance"].fillna(0.0)
        if symbol != SYMBOL_ORDER[0]:
            grouped["time_open"] = grouped["time_open"].fillna(gold_time_open).ffill().bfill()

        bars[symbol] = grouped.reset_index(drop=True)

    log.info("Built aligned bars in %.2fs", time.time() - t0)
    return bars[SYMBOL_ORDER[0]], bars[SYMBOL_ORDER[1]], bars[SYMBOL_ORDER[2]]


def compute_features(df: pd.DataFrame, symbol_index: int) -> np.ndarray:
    df = df.copy()
    ...
    if symbol_index == 0:
        dt = pd.to_datetime(df["time_open"], unit="ms", utc=True)
        ...
        feat["duration_s"] = dt.diff().dt.total_seconds().fillna(0.0)
        feat["rv4"] = ret1.rolling(4, min_periods=4).std(ddof=0)
        feat["rv16"] = ret1.rolling(16, min_periods=16).std(ddof=0)
        ...
        columns = AUX_FEATURE_COLUMNS

    return feat.loc[:, columns].to_numpy(dtype=np.float32, copy=False)
...
def main() -> None:
    ...
    df_gold, df_usdx, df_usdjpy = build_aligned_bars(
        data_path,
    ...
    log.info("Aligned bar count: %d", n_bars)

    x = np.concatenate(
        [
            compute_features(df_gold, symbol_index=0),
            compute_features(df_usdx, symbol_index=1),
            compute_features(df_usdjpy, symbol_index=2),
        ],
        axis=1,
    )
    y = get_triple_barrier_labels(df_gold)
    if x.shape[1] != MODEL_FEATURE_COUNT:
        raise RuntimeError(f"Expected {MODEL_FEATURE_COUNT} features, got {x.shape[1]}")
