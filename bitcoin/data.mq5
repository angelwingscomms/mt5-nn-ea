...
void OnStart() {
   ...
   MqlTick ticks[];
   int total_copied = 0;
   string symbol = (StringLen(export_symbol) > 0) ? export_symbol : _Symbol;
   ...
   while(total_copied < ticks_to_export) {
      ...
      int copied = CopyTicks(symbol, ticks, COPY_TICKS_ALL, last_time, to_copy);
   ...
   FileClose(h);
   PrintFormat("✅ Exported %d ticks for %s.", total_copied, symbol);
}
